...
> Status: pre-release. The production OAuth client is not yet registered with
> the IdP, so `vibeco login --env prod` will not succeed end-to-end until that
> ships. Use `--env local` with a `.env` pointing at your target stack (see
> `.env.example`) in the meantime.

## Install

The repo is currently private and shipped to `vibe-ad` members only. You need
[`gh`](https://cli.github.com/) installed and authenticated (`gh auth login`).

```sh
gh api repos/vibe-ad/vibe-cli/contents/install.sh -H 'Accept: application/vnd.github.raw' | sh
```
...
```sh
gh api repos/vibe-ad/vibe-cli/contents/install.sh -H 'Accept: application/vnd.github.raw' | sh -s -- --no-modify-path
```

Once the repo goes public, the one-liner becomes:
